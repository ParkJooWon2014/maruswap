...
{
	int ret = 0;
	stage_buffer->buffer = kzalloc(STAGE_BUFFER_SIZE,GFP_KERNEL);
	if(!stage_buffer->buffer){
...
int ib_rpc_commit_all(void)
{
	...
	clear_stage_buffer_addr(rdma_ctrl->stage_buffer);
	stage_buffer_count_reset(rdma_ctrl->stage_buffer);
	return ret;
}
...
static int __init init_ib(void)
{
	...
	*/ 
	...
	ret = init_multicast_ctrl(multicast_ctrl);
	if(ret){
		pr_err("Unable to init multicast\n");
		goto out_init_ctrl;
	}

	ret = init_stage_buffer(rdma_ctrl->stage_buffer);
	if(unlikely(ret)){
		pr_err("Unable to init multicast\n");
		goto out_init_ctrl;
	}
	...
	/*
	   ...
	   mbswap_multicast_write(page,rdn[count]);
	   }

	   for(count = 0 ; count < (1UL << 9); count ++){
	   mbswap_rdma_read(page, rdn[count]);
	   }
...
static void __exit cleanup_ib(void)
{
	...
	list_for_each_entry_safe(rdma_ctrl,safe,&ctrl->rdma_ctrl_list,list){	
		if(rdma_ctrl->stage_buffer->buffer){
			kfree(rdma_ctrl->stage_buffer->buffer);
			rdma_ctrl->stage_buffer->buffer = NULL;
			xa_destroy(&rdma_ctrl->stage_buffer->check_list);
		}
		if(rdma_ctrl->mr)
...
int maruswap_rdma_read(struct page *page, u32 offset)
{
	...
	rdma_info = get_rdma_info(rdma_ctrl,nr_memblock);
